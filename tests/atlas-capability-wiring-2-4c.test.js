...
assert.match(hub, /askCompassForSubjects[\s\S]*?'canUseAI'[\s\S]*?'suggest-subject-ideas'/);
...
assert.match(engine, /installSubjectAuthoringAIGuard[\s\S]*?'canUseAI'[\s\S]*?'ai-authoring'/);
...
console.log(
    'Stage 2.4C capability wiring proof passed: fresh AI creation, AI shaping, editing/My Version, and worker bypasses use canonical capabilities.'
);
