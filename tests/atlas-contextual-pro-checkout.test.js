...
assert.match(
  checkout,
  /atlas-pro-checkout-value-title[\s\S]*?Make the lesson[\s\S]*?feel made for them\./,
  'Checkout value reassurance must preserve the intentional two-line checkout composition.'
);
...
assert.match(
  checkout,
  /theme:\s*'light'/,
  'Embedded Paddle checkout must remain light in both Atlas themes for visual stability.'
);

assert.doesNotMatch(
  checkout,
  /checkoutTheme/,
  'Embedded Paddle checkout must not inherit Atlas night mode.'
);
...
assert.match(
  pricing,
  /atlas-pro-checkout\.js\?v=20260925-checkoutfinal1/,
...
assert.match(
  chrome,
  /atlas-account-gate\.js\?v=20260925-checkoutfinal1/
);

assert.match(
  registry,
  /atlas-account-chrome\.js\?v=20260925-checkoutfinal1/
);
